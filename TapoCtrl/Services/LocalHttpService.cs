...
public sealed class LocalHttpService : IDisposable
{
    ...
    private async Task<string> BuildSeriesHtml(string group)
    {
        ...
        async Task<string> makeSeriesJson(IEnumerable<DeviceSnapshot> list,bool humidity=false)
        {
            ...
            {
                var id=humidity ? d.Id+":humidity" : d.Id;
        ...
        "draw('chart1',data1,unit1);"+drawSecond+"</script></body></html>";
    }
    ...
    private async Task<string> BuildGraphHtml(string id)
    {
        ...
        var points=_history is not null ? await _history(device.Id) : [];
        string H(string x)=>WebUtility.HtmlEncode(x??"");
        var json=JsonSerializer.Serialize(points.Select(p=>new{t=p.Time.ToString("yyyy-MM-dd HH:mm:ss"),v=p.Value}));
        var unit=device.Kind switch{DeviceKind.Power=>"W",DeviceKind.Environment=>"℃",DeviceKind.Temperature=>"℃",DeviceKind.Humidity=>"%",_=>""};
        var title=H(device.Name);
        var unitJson=JsonSerializer.Serialize(unit);
        var current=device.Kind switch
        {
            DeviceKind.Power=>$"{device.PowerWatts??0:0} W",
        ...
        sb.AppendLine("<div class='header'><h1>"+title+"</h1><div class='now'>現在 "+H(current)+"</div></div>");
        sb.AppendLine("<div class='panel'>");
        sb.AppendLine("<canvas id='chart' width='1200' height='620'></canvas>");
        sb.AppendLine("<div class='stats' id='stats'></div>");
        sb.AppendLine("<div class='note'>過去24時間の履歴をブラウザ内で表示しています。データが少ない場合は、TapoCtrlをしばらく起動した後に再表示してください。</div>");
        sb.AppendLine("</div>");
        sb.AppendLine("<script>");
        sb.AppendLine("const points="+json+";");
        sb.AppendLine("const unit="+unitJson+";");
        sb.AppendLine("const canvas=document.getElementById('chart');");
        sb.AppendLine("const ctx=canvas.getContext('2d');");
        sb.AppendLine("function draw(){");
        sb.AppendLine("  const w=canvas.width,h=canvas.height,padL=70,padR=25,padT=30,padB=55;");
        sb.AppendLine("  ctx.clearRect(0,0,w,h);");
        sb.AppendLine("  ctx.fillStyle='#15191f';ctx.fillRect(0,0,w,h);");
        sb.AppendLine("  ctx.strokeStyle='rgba(255,255,255,.16)';ctx.lineWidth=1;");
        sb.AppendLine("  const vals=points.map(p=>Number(p.v)).filter(v=>Number.isFinite(v));");
        sb.AppendLine("  const min=vals.length?Math.min(...vals):0, max=vals.length?Math.max(...vals):1;");
        sb.AppendLine("  const span=Math.max(1,max-min), yMin=Math.max(0,min-span*.1), yMax=max+span*.15;");
        sb.AppendLine("  for(let i=0;i<=5;i++){let y=padT+(h-padT-padB)*i/5;ctx.beginPath();ctx.moveTo(padL,y);ctx.lineTo(w-padR,y);ctx.stroke();let v=yMax-(yMax-yMin)*i/5;ctx.fillStyle='#cdd3dd';ctx.font='18px sans-serif';ctx.textAlign='right';ctx.fillText(v.toFixed(1),padL-10,y+6);}");
        sb.AppendLine("  for(let i=0;i<=6;i++){let x=padL+(w-padL-padR)*i/6;ctx.beginPath();ctx.moveTo(x,padT);ctx.lineTo(x,h-padB);ctx.stroke();}");
        sb.AppendLine("  ctx.strokeStyle='#00eaff';ctx.lineWidth=4;ctx.beginPath();");
        sb.AppendLine("  points.forEach((p,i)=>{let x=padL+(w-padL-padR)*(points.length<=1?0:i/(points.length-1));let y=padT+(h-padT-padB)*(1-(Number(p.v)-yMin)/(yMax-yMin)); if(i===0)ctx.moveTo(x,y); else ctx.lineTo(x,y);});");
        sb.AppendLine("  if(points.length)ctx.stroke();");
        sb.AppendLine("  ctx.strokeStyle='#8b94a3';ctx.lineWidth=2;ctx.strokeRect(padL,padT,w-padL-padR,h-padT-padB);");
        sb.AppendLine("  ctx.fillStyle='#e6ebf4';ctx.font='20px sans-serif';ctx.textAlign='center';ctx.fillText(unit,w/2,h-15);");
        sb.AppendLine("  if(!points.length){ctx.fillStyle='#e6ebf4';ctx.font='28px sans-serif';ctx.fillText('履歴データがありません',w/2,h/2);}");
        sb.AppendLine("  const avg=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0;");
        sb.AppendLine("  document.getElementById('stats').innerHTML=[['現在', vals.length?vals[vals.length-1]:0],['最小',min],['最大',max],['平均',avg]].map(x=>'<div class=\"stat\">'+x[0]+'<br><b>'+Number(x[1]).toFixed(1)+' '+unit+'</b></div>').join('');");
        sb.AppendLine("}");
        sb.AppendLine("draw();");
    ...
    private string BuildHtml(IEnumerable<DeviceSnapshot> devices)
    {
        ...
        string card(DeviceSnapshot d)
        {
            ...
            var dbl=canGraph?$" ondblclick=\"openGraph('{H(d.Id)}')\" title='ダブルクリックでグラフを開く'":"";
        ...
        string section(string title,DeviceGroupKind g)
        {
            ...
            var graphAttr=g==DeviceGroupKind.Switch?"":$" ondblclick=\"openSeries('{groupCss(g)}')\" title='ダブルクリックで系列グラフを表示'";
